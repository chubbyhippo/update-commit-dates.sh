...
if [ "$#" -lt 3 ]; then
    echo "Usage: $0 <committer-date> <author-date> <commit-id> [<commit-id> ...]"
    echo "Example: $0 '2023-01-01 12:00:00' '2023-01-01 12:00:00' abc123 def456"
    exit 1
fi
...
# Get the name of the current branch (starting point)
...
for COMMIT_ID in "$@"; do
    ...
    # Checkout the commit and detach HEAD (to rewrite history)
    ...
    git commit --amend --no-edit --date "$AUTHOR_DATE" || exit 1

    # Reapply the rewritten commit to the original branch
    git rebase --onto HEAD "$COMMIT_ID" "$ORIGINAL_BRANCH" || exit 1

    echo "Successfully updated commit $COMMIT_ID with new dates:"
    echo "  Committer Date: $COMMITTER_DATE"
    echo "  Author Date:    $AUTHOR_DATE"

    # Move back to the (possibly updated) branch tip for the next iteration
